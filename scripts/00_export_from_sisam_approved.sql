-- =============================================================================
-- PASO 1: EXPORTAR DE SISAM - SOLO PRODUCTOS CON SOLICITUD APROBADA
-- =============================================================================
...
--   alim_producto (p.id)
--       ↓
--   alim_producto_solicitud (id_alim_producto → id_alim_solicitud)
--       ↓
--   alim_solicitud (id)
--       ↓
--   alim_movimiento_estado_solicitud (estado actual = 8)
-- =============================================================================

-- =============================================================================
-- QUERY DE DIAGNÓSTICO: Verificar distribución de estados
-- =============================================================================
-- Ejecutar primero para entender los datos:

/*
-- Ver todos los estados de solicitud disponibles
SELECT id, nombre, codigo
FROM ctl_estado_solicitud
ORDER BY id;

-- Contar productos por estado actual de su solicitud
...
JOIN alim_producto_solicitud ps ON ps.id_alim_producto = p.id
JOIN alim_solicitud s ON s.id = ps.id_alim_solicitud
JOIN ultimo_estado ue ON ue.id_alim_solicitud = s.id
JOIN ctl_estado_solicitud ces ON ces.id = ue.id_ctl_estado_solicitud
WHERE p.estado_registro = 1
GROUP BY ces.id, ces.nombre
ORDER BY ces.id;
*/

-- =============================================================================
-- QUERY PRINCIPAL: Extraer productos con solicitud APROBADA (estado 8)
-- =============================================================================
...
WITH ultimo_estado_solicitud AS (
    ...
    SELECT DISTINCT ON (mes.id_alim_solicitud)
        mes.id_alim_solicitud,
        mes.id_ctl_estado_solicitud,
        mes.fecha as fecha_ultimo_estado
    FROM alim_movimiento_estado_solicitud mes
    ORDER BY mes.id_alim_solicitud, mes.fecha DESC
),
...
SELECT DISTINCT ON (p.id)
    ...
    -- ==========================================================================
    -- Relaciones (IDs de origen para cruce con Centro de Datos)
    -- ==========================================================================
    p.id_sub_grupo_alimenticio as original_sub_id,
    clv.id as original_clv_id,
    -- Campos de cruce para PAISES (múltiples opciones)
    pais.isonumero as original_pais_iso_number,      -- Cruce principal: iso_number
    pais.dominio2 as original_pais_iso_2,            -- Fallback: iso_2_code
    pais.dominio3 as original_pais_iso_3             -- Fallback: iso_3_code
FROM alim_producto p
-- ⭐ FILTRO PRINCIPAL: Solo productos con solicitud aprobada
INNER JOIN productos_aprobados pa ON pa.id_alim_producto = p.id
-- JOINs existentes
...
/*
-- Productos totales activos (sin filtro de solicitud)
SELECT 'Sin filtro de solicitud' as criterio, COUNT(DISTINCT id) as productos
FROM alim_producto
WHERE estado_registro = 1
  AND fecha_emision_registro IS NOT NULL
  AND fecha_vigencia_registro IS NOT NULL;

-- Productos con solicitud aprobada (estado 8)
WITH ultimo_estado_solicitud AS (
    SELECT DISTINCT ON (mes.id_alim_solicitud)
        mes.id_alim_solicitud,
        mes.id_ctl_estado_solicitud
...
SELECT 'Con solicitud aprobada (estado 8)' as criterio, COUNT(DISTINCT p.id) as productos
FROM alim_producto p
JOIN alim_producto_solicitud ps ON ps.id_alim_producto = p.id
...
WHERE p.estado_registro = 1
  AND p.fecha_emision_registro IS NOT NULL
  AND p.fecha_vigencia_registro IS NOT NULL
  AND ues.id_ctl_estado_solicitud = 8;
*/

-- =============================================================================
-- INSTRUCCIONES:
-- =============================================================================
-- 1. Ejecutar primero el query de diagnóstico (descomentarlo)
-- 2. Verificar que el estado 8 corresponde a "Aprobada"
-- 3. Ejecutar el query principal
-- 4. En TablePlus: Click derecho en el resultado → Export → CSV
-- 5. Importar en Core usando: 01_create_temp_table.sql + COPY o TablePlus
-- 6. Ejecutar: 03_migrate_by_entity_uuid.sql
-- =============================================================================
